...
async def build_poseidon_report(windy_data: Dict, location: str, date: str) -> str:
    ...
    # Формируем отчет БЕЗ f-строки с обратными слешами
    # Используем обычную конкатенацию строк
    report_lines = [
        "🔱 ПОСЕЙДОН ВНЯЛ ТВОИМ МОЛИТВАМ 🙏🏻",
        "",
        f"На {date.split('-')[2]} ноября {location} готовит сюрприз. Лови мой вердикт, не перебивай.",
        "",
        f"ВОЛНА: {calculate_ranges(wave_data)}м",
        wave_comment,
        "",
        f"ПЕРИОД: {calculate_ranges(period_data)} сек", 
        period_comment,
        "",
        ...
        "ПРИЛИВЫ/ОТЛИВЫ:",
        "",
        f"· Приливы: {tides['high_times'][0]} ({tides['high_heights'][0]}м) и {tides['high_times'][1]} ({tides['high_heights'][1]}м)",
        f"· Отливы: {tides['low_times'][0]} ({tides['low_heights'][0]}м) и {tides['low_times'][1]} ({tides['low_heights'][1]}м)",
        "",
        "ВЕРДИКТ ПО ВРЕМЕНИ:",
        ""
    ]
    
    # Добавляем временные периоды
